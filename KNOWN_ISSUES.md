...
| Support address not configured | **resolved 2026-09-19** | The website now gives contact@personewsap.com on /support, /privacy, /delete-account and the site footer, from one constant (`src/lib/contact.ts`); `VITE_SUPPORT_EMAIL` was removed. The mobile app's support screen still defaults to support@personewsap.com (`apps/mobile/app/support.tsx`). |
...
| Scored-question preflight and contract declaration not applied to staging | open | `supabase-staging/.../20260906110000_scored_question_preflight.sql` adds the question contract, the legacy cutover and the gate that refuses a batch whose questions are wrong; `20260907180000_scored_question_contract_declaration.sql` makes the canonical payload DECLARE `scored_questions_required` and `scored_question_contract_version`, which is what production reads instead of inferring. Until both are applied, an edition with no questions still publishes AND production has nothing to hold it to. The cutover lives in exactly one place — `scored_question_cutover_edition()`, default `2026-09-09` — and is movable with `app.scored_question_cutover_edition`; production has no second date of its own. Validate with `npm run publisher:test:sql:dry`. |
| Generators not yet emitting scored questions; the 2026-09-09 batch is blocked by design | open | The contract reaches the Scheduled Tasks through the bridge manifest and the prompts carry it, but the prepared 2026-09-09 batch (23 jobs: 16 newsletters, 1 story, 6 mini cases) predates it. Its approved Business Story carries no questions at all, and its six approved mini cases carry the old shape — role `framework`, options graded with `is_correct`. Both are refused by `assert_edition_questions_publishable`, per job and with the failing job id, so no payload is offered and nothing publishes half-right. That is the intended outcome: the edition stays blocked until the outputs are regenerated against the contract. Nothing in the code repairs a reviewed output in place, auto-approves a review, or bypasses the gate for a date — and the gate suite's N-series asserts that the historical outputs and reviews are left untouched. |
...
## Resolved on 2026-09-07 — the production verification could report a false green

The single most consequential defect found in this repository, and the one that
would have been hardest to notice in production: a total failure of the question
stage was indistinguishable from a legacy edition, and verified clean.

| Defect | Where | Fix |
| --- | --- | --- |
| A questions-required edition whose question stage wrote NOTHING verified `ok: true` | `edition_expects_questions` (20260906105000) answered "does this edition owe its readers questions?" with `exists (content_items join logical_questions …)`. It inferred the REQUIREMENT from the PERSISTENCE, so zero questions read as "none were expected". Verification returned `questions_not_expected`, staging wrote a `published` receipt, the gate stopped re-offering the batch, and the edition was live and unplayable — permanently, because a receipt is what makes a retry a no-op | the requirement is now a DECLARATION. Staging stamps `scored_questions_required` and `scored_question_contract_version` onto the canonical payload's `batch`; the production publisher stamps it onto every item it writes, inside the publishing transaction, as `metadata.staging_scored_question_contract`. `edition_question_contract` reads only that. Zero questions on a declared edition now fails with `questions_missing_entirely` |
| "No declaration" silently meant "no questions expected" | there was no way to tell a pre-contract item from an item whose payload declared nothing | three structural states, and no second cutover date anywhere: the metadata key ABSENT means written before the contract existed (genuinely legacy, and a state no new publish can re-create, because the patched publisher always stamps it); `version = 0` means a contract-aware publisher was told nothing, which FAILS as `contract_declaration_missing`; `version >= 1` is authoritative. A batch mixing them fails as `contract_declaration_inconsistent` |
| The verification checked counts, not the contract | roles, the exact 0/300/600/1000 tier set and option label locales were unchecked, so a mini case with the old `framework` role, a question with two options at 1000 and no 300, or an option with no French label could all have passed | `verify_scheduled_edition_game` now checks all of them, still counting the private grade rows and returning none. Its `problems` are ordered most-fundamental-first so the headline `reason` is deterministic |
| The staging question gate passed a questions-required batch with nothing to check | `jobs_checked = 0` left `v_blockers` empty, so `ok` was true; only the editorial gate next door refused it. A gate whose verdict is correct because a different gate is also running is not a gate | `scored_questions_absent` blocker |
| The contract version existed as a string in one place and as nothing in the other | `scored_question_contract()` reported the literal `scored-questions-v1`; production had no notion of a version at all | one integer, `scored_question_contract_version()`, in both projects. The staging contract derives its version string from it, the payload declares it, and production refuses a payload declaring a version it does not implement |

Proven by `supabase/tests/edition_question_contract.test.sql` (47 checks) and the
new M/N/O/P sections of the staging gate suite. Restoring the old inference
makes the A-series, D7–D11 and F1 fail with the exact `questions_not_expected`
the defect produced.
