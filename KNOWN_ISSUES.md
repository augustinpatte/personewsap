...
| Teams migrations still pending everywhere | open | The twelve `20260906090000`–`20260906105000` migrations have never been applied to staging or production. Validate them first with `npm run teams:test:sql -- --with-migrations`, which inlines them into a transaction that rolls back. |
