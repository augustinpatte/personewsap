...
async function main(): Promise<void> {
  ...
  if (command === "push-receipts") {
    const output = await runPushReceipts(parsePushReceiptsOptions(args));
    writeJson(output, { redactIdentifiers: true });
    return;
  }
...
Commands:
  ...
  push-notifications      Send due edition notifications (20:00 reader-local) and next-morning answer reminders (08:30 reader-local). Idempotent per device.
