...
-- The edition fixture lives in tests/lib/edition_fixture.sql and is
-- concatenated ahead of this file into the same psql session by
-- scripts/local-sql-tests.mjs. It is shared with the publication E2E so both
-- are written against one definition of what an edition looks like.
--
-- The results table stays here: it belongs to this suite's report, not to the
-- fixture.
